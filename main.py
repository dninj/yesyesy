...
""")
    info(message)#запускает бота
    
@bot.message_handler(commands=['info ℹ'])#показывает информацию об командах бота
def info(message):
...
/slills - показывает какой язык вы использовали

/start - запускает бота

/info - показывает что делает та или другая программа

/projects - показывает все твои проекты 

/delete - удаляет твой проект

/update_projects - позволяет обновитьй твой проект
....

Также ты можешь ввести имя проекта и узнать информацию о нем!""")
    

@bot.message_handler(commands=['new_project 📽📽'])#создаёт новый проект
def addtask_command(message):
    bot.send_message(message.chat.id, "Введите название проекта:")
    bot.register_next_step_handler(message, name_project)#позволяет добавить имя проекту
...
def name_project(message):
    ...
    bot.register_next_step_handler(message, link_project, data=data)#позволяет добавить ссылку на проект
...
def callback_project(message, data, statuses):
    ...
    if status not in statuses:
        ...
        bot.register_next_step_handler(message, callback_project, data=data, statuses=statuses)#отзывает твой проект
        return
...
@bot.message_handler(commands=['skills 🤹‍♀️🤹‍♀️'])#позволяет тебе показать какие языки ты использовал
def skill_handler(message): # отправляет сообщение
    user_id = message.from_user.id
    ...
    if projects:
        ...
        bot.register_next_step_handler(message, skill_project, projects=projects)#отоброжает какие ты указал скилы в проекте  
    else:
        no_projects(message)
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        ...
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)#позволяет установить скилл
...
@bot.message_handler(commands=['projects 😎😋'])#позволяет посмотреть твои проекты
...
@bot.message_handler(commands=['delete ⛔'])# удалить проект
...
@bot.message_handler(commands=['update_projects 👶'])#обновляет твой проект
